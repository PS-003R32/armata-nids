#include <a111125_inferencing.h>
#include <Wire.h>
#include <LiquidCrystal_I2C.h>
#include <Adafruit_GFX.h>
#include <Adafruit_SSD1306.h>

#define EI_FEATURE_COUNT 10 
...
void loop() {
  if (stringComplete) {
    ...
    if (parseSuccess) {
      ...
      // Updated this condition from the last pico_v1 code to not refresh the display with the sam attack lable. 
      if (max_label != lastDisplayedLabel) {
        updateDisplays(max_label, max_score);
        lastDisplayedLabel = max_label; 
      }
...
void updateDisplays(String label, float score) {
  ...
  if (label != "BENIGN") {
    ...
    display.println("Last seen: " + attackerIP);
    display.println("Last type: BENIGN");
    display.display();
  }
}
